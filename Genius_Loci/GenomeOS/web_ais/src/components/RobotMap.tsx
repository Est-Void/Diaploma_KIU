...
const WAREHOUSE_ZONES = {
  racks: [
    { name: 'Storage Rack A', bounds: [[-35, -40], [-25, -20]] as L.LatLngBoundsExpression, color: '#64748b' },
    { name: 'Storage Rack B', bounds: [[-35, -15], [-25, 5]] as L.LatLngBoundsExpression, color: '#64748b' },
    { name: 'Storage Rack C', bounds: [[-35, 10], [-25, 30]] as L.LatLngBoundsExpression, color: '#64748b' },
    { name: 'Storage Rack D', bounds: [[-10, -40], [0, -20]] as L.LatLngBoundsExpression, color: '#64748b' },
    { name: 'Storage Rack E', bounds: [[-10, -15], [0, 5]] as L.LatLngBoundsExpression, color: '#64748b' },
    { name: 'Storage Rack F', bounds: [[-10, 10], [0, 30]] as L.LatLngBoundsExpression, color: '#64748b' },
    { name: 'Storage Rack G', bounds: [[15, -40], [25, -20]] as L.LatLngBoundsExpression, color: '#64748b' },
    { name: 'Storage Rack H', bounds: [[15, -15], [25, 5]] as L.LatLngBoundsExpression, color: '#64748b' },
    { name: 'Storage Rack I', bounds: [[15, 10], [25, 30]] as L.LatLngBoundsExpression, color: '#64748b' },
  ],
  docks: [
    { name: 'Loading Dock 1', bounds: [[-45, 35], [-38, 48]] as L.LatLngBoundsExpression, color: '#f59e0b' },
    { name: 'Loading Dock 2', bounds: [[38, 35], [45, 48]] as L.LatLngBoundsExpression, color: '#f59e0b' },
  ],
  charging: [
    { name: 'Charging Station', bounds: [[38, -48], [45, -35]] as L.LatLngBoundsExpression, color: '#10b981' },
  ],
  obstacles: [
    { name: 'Pillar 1', center: [-20, -20] as L.LatLngExpression, radius: 2 },
    { name: 'Pillar 2', center: [20, 20] as L.LatLngExpression, radius: 2 },
  ]
}
...
const RobotMap: React.FC<Props> = ({ robots, fullSize }) => {
  ...
  useEffect(() => {
    ...
    const warehouseBounds: L.LatLngBoundsExpression = [[-50, -50], [50, 50]]
    ...
    for (let i = -50; i <= 50; i += 10) {
      L.polyline([[i, -50], [i, 50]], { color: '#e2e8f0', weight: 0.5, dashArray: '4,4' }).addTo(map)
      L.polyline([[-50, i], [50, i]], { color: '#e2e8f0', weight: 0.5, dashArray: '4,4' }).addTo(map)
    }

    // Coordinate labels
    for (let i = -40; i <= 40; i += 20) {
      L.marker([i, -52], { icon: L.divIcon({
        className: 'coord-label',
        html: `<span style="font-size:10px;color:#94a3b8">${i}m</span>`,
        iconSize: [30, 12]
      })}).addTo(map)
      L.marker([-52, i], { icon: L.divIcon({
        className: 'coord-label',
        html: `<span style="font-size:10px;color:#94a3b8">${i}m</span>`,
        iconSize: [30, 12]
      })}).addTo(map)
    ...
    // Aisle labels
    const aisles = [
      { pos: [30, -30] as L.LatLngExpression, text: 'Main Aisle A' },
      { pos: [30, 15] as L.LatLngExpression, text: 'Main Aisle B' },
      { pos: [-30, -30] as L.LatLngExpression, text: 'Aisle C' },
      { pos: [-30, 15] as L.LatLngExpression, text: 'Aisle D' },
    ]
