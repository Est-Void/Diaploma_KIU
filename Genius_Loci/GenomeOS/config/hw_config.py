...
NODES_CONFIG = {
    ...
    "balancer": {
        ...
        "perception": {
        "obstacle_disparity_threshold": 80, # При каком значении disparity считать объект препятствием
        "grab_allowed_distance_m": 0.65,    # На каком расстоянии разрешать команду "Хватай"
        "grab_alignment_tolerance": 0.1     # Допустимое отклонение груза от центра (в долях от кадра)
    },
        "mqtt": {
        "broker_address": "127.0.0.1", # IP адрес сервера (для тестов на ПК - локальный)
        "broker_port": 1883,
        "robot_id": "rover_01",         # Уникальный ID нашего робота
        "keepalive": 60,                # Пинг каждые 60 сек
        "qos": 1,                       # Quality of Service (1 = минимум один раз доставится)
        "telemetry_interval_sec": 1.0   # Как часто слать координаты и состояние
    }
}