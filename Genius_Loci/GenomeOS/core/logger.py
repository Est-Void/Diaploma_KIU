"""
Structured logging module for GenomeOS.
...
def setup_logging(
    ...
    root_logger = logging.getLogger("GenomeOS")
...
def get_logger(name: str, context: Optional[Dict[str, Any]] = None) -> ContextAdapter:
    ...
    logger = logging.getLogger(f"GenomeOS.{name}")

    if name not in _loggers:
        _loggers[name] = logger

    ctx = context or {}
    ctx.setdefault("robot_id", "genius_loci_001")
